...
#define L1_SETS 256
#define L1_SET_BITS 8// note: this is log2Ceil(L1_SETS)
#define L1_WAYS 8 //note: this looks like there are 8 ways for CVA6
//#define L1_WAYS 4 //note: this looks like there are 4 ways for medium and small boom
#define L1_BLOCK_SZ_BYTES 16
#define L1_BLOCK_BITS 4 // note: this is log2Ceil(L1_BLOCK_SZ_BYTES)
#define L1_SZ_BYTES (L1_SETS*L1_WAYS*L1_BLOCK_SZ_BYTES) //32768
...
//     for(int i = 0 ; i < 5*L1_SZ_BYTES ; i++){
//         dummyMem[i] = dummyMem[i] + 0;
//     }
// }
void flushCache(uint64_t addr, uint64_t sz){
    ...
    for (uint64_t i = 0; i < numSetsClear; ++i){
        ...
        for(uint64_t j = 0; j < 4*L1_WAYS; ++j){
            ...
            //printbits(alignedMem+setOffset+wayOffset);
        }
    }
}
